...
const MAX_COLORS = 8;
const SESSION_START_KEY = 'color-bends-start-ms';
...
export default function ColorBends({
  ...
  useEffect(() => {
    ...
    const storedStart = Number(window.sessionStorage.getItem(SESSION_START_KEY));
    const startMs = Number.isFinite(storedStart) && storedStart > 0 ? storedStart : fallbackNow;
    if (!Number.isFinite(storedStart) || storedStart <= 0) {
      window.sessionStorage.setItem(SESSION_START_KEY, String(startMs));
    }
  ...
  useEffect(() => {
    ...
    const arr = palette.filter(Boolean).slice(0, MAX_COLORS).map(toVec3);
